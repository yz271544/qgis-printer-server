                ...
                for (QObject *obj: legend->model()->rootGroup()->children()) {
                    ...
                    if (tr) {
                        ...
                        if (layer_custom_type == "point" && tr->layerId() == layer->id()) {
                            spdlog::debug("layer name: {}, layer_custom_type: {}, tr layer name: {}",
                                          layer->name().toStdString(), layer_custom_type.toStdString(),
                                          tr->name().toStdString());
                            // custom symbol
                            // 遍历图例的符号项
                            for (const QgsLegendSymbolItem& legend_symbol_item: legend_symbol_items) {
                                ...
                                for (QgsSymbolLayer *symbol_layer: symbol_layers) {
                                    ...
                                    if (symbol_layer->layerType() == "FontMarker") {
                                        ...
                                        cloned_symbol_layer->setEnabled(false);
                                        filtered_symbol_layers.append(cloned_symbol_layer);
                                    } else {
                                        auto cloned_symbol_layer = symbol_layer->clone();
                                        filtered_symbol_layers.append(cloned_symbol_layer);
                                    }
                            ...
                            spdlog::debug("line: {}, legend_node_orders: {}", layer->name().toStdString(),
                                          ShowDataUtil::formatQListIntToString(legend_node_orders));
                            ...
                            for (QgsLegendSymbolItem legend_symbol_item: legend_symbol_items) {
                                ...
                                // 设置图例节点的自定义符号
                                QgsMapLayerLegendUtils::setLegendNodeCustomSymbol(
                                    tr, 0, filtered_line_symbol.release());
                            }
                        }
                    }
                    legend->model()->refreshLayerLegend(tr);
                }
            }
        }
    }

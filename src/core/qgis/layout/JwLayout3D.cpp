...
LookAtPoint *JwLayout3D::set3DCanvasCamera(
  ...
  float distance =
      calculate_opposite_side(camera->cameraHeight, std::abs(pitch));
  spdlog::info("distance: {}", distance);

  // 计算倾斜后向下拉回的偏移offset_pull_pitch度的距离
  double offsetDir = calculate_opposite_side(
 ...
 * @return 临边长度
 */
double JwLayout3D::calculateAdjacentSide(double cameraHeight,
                                         double pitchDegrees) {
  // 将角度转换为弧度
 ...
 * @param distance 斜边长度
 */
double JwLayout3D::calculate_opposite_side(double distance,
                                           double pitchDegrees) {
  // 将角度转换为弧度
